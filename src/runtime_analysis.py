...
def get_runtime_results(graph):
    ...
    result_dict = {
        ...
        'avg_num_neighbors' : np.mean([len(list(env.g.neighbors(node))) for node in env.g.nodes()])
        }
    ...
    result_dict['Mean GARDEN Overhead'] = np.mean(1000 * (end - start) / len(env.g.nodes()))
    
    for agent in [agent_gnn, agent_mlpwd, agent_mlp, greedy_walker, distance_walker, connection_walker, random_walker]:
        result_dict[f"Mean Action Time: {agent.name}"] = get_action_times(agent, env, test_set)
...
if __name__ == "__main__":
    ...
    res = []
    graphs = ["facebook_0", "facebook_348", "facebook_686", "facebook_3437", "facebook_414"]
    ...
    results.sort_values(by='avg_num_neighbors').to_csv("results/runtime_analysis.csv")